...
async function createCandidate(data) {
  ...
  // Use environment variable for default password, generate random if not set
  const defaultPassword = process.env.DEFAULT_CANDIDATE_PASSWORD || require('crypto').randomBytes(16).toString('base64');
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // Primary division stored on user record
  ...
  logger.info(`Created candidate with division: ${divisionValue}`);
  logger.info(`Created candidate with positionAppliedFor:`, JSON.stringify(positionAppliedFor));
 ...
 */
async function searchCandidates(filters, pagination) {
